...
#[test]
fn checking_out_an_already_loaned_item_fails() {
    let mut library = library_with_items();
...
fn an_ebook_returned_late_still_owes_nothing() {
    ...
    // Item 4 is an ebook — always 0 owed, no matter how late.
...
#[test]
fn checking_out_an_unknown_item_is_rejected() {
    let mut library = library_with_items();
...
#[test]
fn checking_out_to_an_unknown_member_is_rejected() {
    let mut library = library_with_items();
...
#[test]
fn checking_out_a_lost_item_is_rejected() {
    let mut library = Library::new();
    let mut item = Item::new(1, "Dune".into(), "Frank Herbert".into(), MediaKind::Book { pages: 320 });
    item.status = LoanStatus::Lost;
    library.add_item(item).unwrap();
    library.register_member(Member::new(100, "Ada".into())).unwrap();
