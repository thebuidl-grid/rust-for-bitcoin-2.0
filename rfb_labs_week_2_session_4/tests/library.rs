...
fn returning_on_time_owes_nothing() {
    ...
    // A book may be kept 21 days; returning on day 15 is on time.
    library.checkout(1, 100, 0).unwrap();

    assert_eq!(library.return_item(1, 15), Ok(0));
}
...
fn an_ebook_returned_late_still_owes_nothing() {
    ...
    // An ebook may be kept 7 days; returning on day 100 is very late, but
    // ebooks are never charged a fee.
...
#[test]
fn return_item_rejects_a_return_day_earlier_than_the_borrow_day() {
    let mut library = library_with_items();
