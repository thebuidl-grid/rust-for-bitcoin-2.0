...
fn an_on_time_return_owes_nothing() {
    ...
    // A book may be kept 21 days; returning on day 20 is on time.
    library.checkout(1, 100, 0).unwrap();
...
fn an_ebook_returned_late_still_owes_nothing() {
    ...
    // Ebooks may be kept 7 days; returning on day 100 is very late, but
...
// `Item`'s fields are public, so a test can build an item that starts out
// lost without the library needing to expose a way to mark one lost.
fn lost_item(id: u32) -> Item {
...
fn checkout_rejects_a_lost_item() {
    let mut library = library_with_items();
    library.add_item(lost_item(5)).unwrap();
...
#[test]
fn return_item_rejects_an_item_that_is_not_on_loan() {
    let mut library = library_with_items();
