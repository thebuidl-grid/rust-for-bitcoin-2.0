    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            LibraryError::ItemIsLost { id } => {
                write!(formatter, "item {id} is marked as lost and cannot be lent")
            }
            LibraryError::BorrowLimitReached { member_id, limit } => {
                write!(
                    formatter,
                    "member {member_id} has reached the borrow limit of {limit} item(s)"
                )
            }
