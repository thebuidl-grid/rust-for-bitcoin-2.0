    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            ...
            LibraryError::DuplicateItemId{id} => {
                write!(formatter, "DuplicateItemId: The item id: {id} already exist")
            },
            LibraryError::DuplicateMemberId{id} => {
                write!(formatter, "DuplicateMemberId: The member id: {id} already exist")
            },
            LibraryError::ItemNotFound{id} => {
                write!(formatter, "ItemNotFound: The item id: {id} does not exist")
            },
            LibraryError::MemberNotFound{id} => {
                write!(formatter, "MemberNotFound: The member id: {id} does not exist")
            },
            LibraryError::ItemAlreadyOnLoan{id, member_id} => {
                write!(formatter, "ItemAlreadyOnLoan: The item id: {id} has being loaned to member id: {member_id}")
            },
            ...
            LibraryError::BorrowLimitReached{member_id, limit} => {
                write!(formatter, "BorrowLimitReached: The member id : {member_id} has reached borrow limit of {limit}")
            },
            LibraryError::InvalidReturnDay{day_borrowed, day_returned} => {
                write!(formatter, "InvalidReturnDay: Item borrowed on {day_borrowed} was returned on {day_returned}")
            }
        }
    }
}
