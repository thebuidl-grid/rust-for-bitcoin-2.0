...
impl Library {
    ...
    pub fn longest_loan_item(&self) -> Option<&Item> {
        self.items.iter().max_by_key(|it| it.loan_days())
    }
    ...
    pub fn checkout(&mut self, item_id: u32, member_id: u32, day: u32) -> Result<(), LibraryError> {
        ...
        // validate first
        if self.items[item_pos].status == LoanStatus::Lost {
            return Err(LibraryError::ItemIsLost { id: item_id });
        }
        if let LoanStatus::OnLoan {
            member_id: other, ..
        } = self.items[item_pos].status
        {
        ...
        if self.members[member_pos].borrowed_item_ids.len() >= MAX_ITEMS_PER_MEMBER {
        ...
        // mutate second
        self.items[item_pos].status = LoanStatus::OnLoan {
            member_id,
            day_borrowed: day,
        };
        self.members[member_pos].borrowed_item_ids.push(item_id);
    ...
    pub fn return_item(&mut self, item_id: u32, day: u32) -> Result<u32, LibraryError> {
        ...
        let (member_id, day_borrowed) = match self.items[item_pos].status {
            LoanStatus::OnLoan {
                member_id,
                day_borrowed,
        ...
        let fee = self.items[item_pos].late_fee_cents(days_held);

        self.items[item_pos].status = LoanStatus::Available;
        ...
        if let Some(idx) = self.members[member_pos]
            .borrowed_item_ids
            .iter()
            .position(|&id| id == item_id)
        {
            self.members[member_pos].borrowed_item_ids.remove(idx);
        }
