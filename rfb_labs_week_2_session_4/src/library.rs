...
impl Library {
    ...
    pub fn checkout(
        ...
        // --- Validate first, in the required order ---
        ...
        // --- All checks passed: mutate second ---

        let item = self
            .items
            .iter_mut()
            .find(|item| item.id == item_id)
            .expect("item existence was already confirmed above");
        ...
        let member = self
            ...
            .iter_mut()
            .find(|member| member.id == member_id)
            .expect("member existence was already confirmed above");
