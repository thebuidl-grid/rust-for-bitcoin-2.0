...
/// item's `LoanStatus` and a member's borrowed-id list in agreement.
...
impl Library {
    ...
    pub fn longest_loan_item(&self) -> Option<&Item> {
        self.items.iter().fold(None, |best, item| match best {
            None => Some(item),
            Some(current) if item.loan_days() > current.loan_days() => Some(item),
