...
impl Library {
    ...
    pub fn add_item(&mut self, item: Item) -> Result<(), LibraryError> {
        ...
        if self.items.iter().any(|stored| stored.id == item.id) {
    ...
    pub fn register_member(&mut self, member: Member) -> Result<(), LibraryError> {
        if self.members.iter().any(|stored| stored.id == member.id) {
    ...
    pub fn longest_loan_item(&self) -> Option<&Item> {
        self.items.iter().fold(None, |longest, item| match longest {
            Some(current) if current.loan_days() >= item.loan_days() => Some(current),
            _ => Some(item),
        })
    }
