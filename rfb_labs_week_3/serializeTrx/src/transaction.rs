...
pub fn hex_to_bytes(hex: &str) -> Result<Vec<u8>, String> {
    if hex.len() % 2 != 0 {
        return Err(format!("Hex string must have even length, got {} characters", hex.len()));
    }

    if let Some(bad) = hex.chars().find(|c| !c.is_ascii_hexdigit()) {
        return Err(format!("Invalid character '{}' (must be 0-9/a-f/A-F)", bad));
    }
    ...
    for i in (0..hex.len()).step_by(2) {
        ...
        // charset already validated above, so this cannot fail
        let byte = u8::from_str_radix(&hex[i..i + 2], 16)
            .map_err(|e| format!("failed to parse hex byte '{}': {}", &hex[i..i + 2], e))?;
...
pub fn validate_txid(bytes: &[u8]) -> Result<(), String> {
    if bytes.len() != 32 {
        return Err(format!("txid must be exactly 32 bytes (64 hex chars), got {} bytes", bytes.len()));
    }
    Ok(())
}