...
pub fn compatibility_report(capabilities: SenderCapabilities) -> CompatibilityReport {
    CompatibilityReport {
        // Legacy P2PKH addresses begin with `1` on Bitcoin mainnet.
        p2pkh: can_send_to(capabilities, AddressFormat::P2pkh),

        // P2SH-wrapped SegWit addresses usually begin with `3`.
        // The sender only needs P2SH/Base58Check support to pay them.
        p2sh_p2wpkh: can_send_to(capabilities, AddressFormat::P2sh),

        // Native SegWit v0 uses Bech32 addresses beginning with `bc1q`.
        p2wpkh: can_send_to(capabilities, AddressFormat::P2wpkh),

        // Taproot uses SegWit v1 and Bech32m addresses beginning with `bc1p`.
