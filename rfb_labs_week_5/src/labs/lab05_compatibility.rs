...
pub fn compatibility_report(capabilities: SenderCapabilities) -> CompatibilityReport {
    CompatibilityReport {
        ...
        // Wrapped SegWit hides a witness program behind an ordinary `3`/`2` address,
        // so a sender needs nothing beyond Base58Check P2SH support.
...
pub fn best_supported_format(capabilities: SenderCapabilities) -> Option<AddressFormat> {
    // Cheapest and most modern first, falling back one generation at a time.
    [
        AddressFormat::P2tr,
...
pub fn required_encoding(format: AddressFormat) -> &'static str {
    match format {
        ...
        // BIP350 changed the checksum constant for witness version 1 and above, so a
        // Bech32-only decoder rejects a valid Taproot address outright.
