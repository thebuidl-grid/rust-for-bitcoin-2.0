...
pub fn build_2_of_3_redeem_script(public_keys: [&str; 3]) -> LabResult<String> {
    let [key1, key2, key3] = public_keys;
    let key1 = parse_public_key(key1)?;
    let key2 = parse_public_key(key2)?;
    let key3 = parse_public_key(key3)?;
...
pub fn derive_p2sh_address(redeem_script_hex: &str, network: Network) -> LabResult<String> {
    let script = parse_redeem_script(redeem_script_hex)?;
