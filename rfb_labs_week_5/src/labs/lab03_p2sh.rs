...
fn parse_public_keys(public_keys: [&str; 3]) -> LabResult<[PublicKey; 3]> {
    let keys: Vec<PublicKey> = public_keys
        .iter()
        .map(|key| {
            PublicKey::from_str(key).map_err(|error| LabError::InvalidKey(error.to_string()))
        })
        .collect::<Result<_, _>>()?;
