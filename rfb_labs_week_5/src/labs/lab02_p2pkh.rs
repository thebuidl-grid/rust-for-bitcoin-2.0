...
fn parse_public_key(public_key_hex: &str) -> LabResult<PublicKey> {
    return PublicKey::from_str(public_key_hex)
...
pub fn p2pkh_spend_template(
    ...
    let public = parse_public_key(public_key_hex)?;
    return Ok(P2pkhSpendTemplate {
        script_sig_items: vec![signature_hex.to_owned(), public.to_string()],
