...
pub fn identify_prefix(address: &str) -> AddressFormat {
    ...
    if lower.starts_with('1') || lower.starts_with('m') || lower.starts_with('n') {
        AddressFormat::P2pkh
...
pub fn inspect_address(address: &str, network: Network) -> LabResult<AddressReport> {
    ...
    let unchecked = bitcoin::Address::from_str(address)
        .map_err(|e| crate::error::LabError::InvalidAddress(e.to_string()))?;
    let checked = unchecked
        .require_network(network)
        .map_err(|e| crate::error::LabError::WrongNetwork(e.to_string()))?;
