...
pub fn transaction_weight(stripped_size: u64, total_size: u64) -> LabResult<u64> {
    ...
    stripped_size
        .checked_mul(3)
        .and_then(|value| value.checked_add(total_size))
        .ok_or_else(|| LabError::InvalidSize("transaction weight overflow".to_owned()))
}
...
pub fn fee_sats(vbytes: u64, feerate_sat_vb: u64) -> LabResult<u64> {
    vbytes
        ...
        .ok_or_else(|| LabError::InvalidSize("transaction fee overflow".to_owned()))
}
...
pub fn compare_fees(
    ...
    let savings_sats = legacy_fee_sats
        ...
        .ok_or_else(|| {
            LabError::InvalidSize(
                "SegWit fee cannot exceed legacy fee when calculating savings".to_owned(),
            )
        })?;
