...
pub fn transaction_weight(stripped_size: u64, total_size: u64) -> LabResult<u64> {
    if stripped_size > total_size {
        return Err(crate::LabError::InvalidSize(
            "stripped size cannot exceed total size".to_string(),
        ));
    }

    let stripped_weight = stripped_size
        .checked_mul(3)
        .ok_or_else(|| crate::LabError::InvalidSize("weight calculation overflow".to_string()))?;
...
pub fn fee_sats(vbytes: u64, feerate_sat_vb: u64) -> LabResult<u64> {
    vbytes
        ...
        .ok_or_else(|| crate::LabError::InvalidSize("fee calculation overflow".to_string()))
}
...
pub fn compare_fees(
    ...
    let savings_sats = legacy_fee_sats
        ...
        .ok_or_else(|| crate::LabError::InvalidSize("SegWit fee exceeds legacy fee".to_string()))?;
