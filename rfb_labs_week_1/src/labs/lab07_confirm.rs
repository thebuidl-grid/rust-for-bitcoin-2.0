...
pub fn confirm_and_locate_transaction<C: RpcClient>(
    ...
    let confirmations = value["confirmations"]
        .as_i64()
        .ok_or(LabError::MissingField("confirmations"))?;
    let block_hash = value["blockhash"]
        .as_str()
        .ok_or(LabError::MissingField("blockhash"))?
        .to_string();

    let block_raw = client.call(None, "getblock", &[block_hash.clone(), "1".to_string()])?;
    let block_value: Value =
        serde_json::from_str(&block_raw).map_err(|e| LabError::Parse(e.to_string()))?;
    let transaction_is_in_block = block_value["tx"]
        .as_array()
