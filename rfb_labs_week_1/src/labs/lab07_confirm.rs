...
pub fn confirm_and_locate_transaction<C: RpcClient>(
    ...
    // One `gettransaction` supplies both the depth and the containing block hash.
    let raw = client.call(Some(wallet_name), "gettransaction", &[txid.to_owned()])?;
    let status = parse_cli_value(&raw)?;
    let confirmations = status
        .get("confirmations")
        .and_then(Value::as_i64)
        .ok_or(LabError::MissingField("confirmations"))?;
    let block_hash = required_string(&status, "blockhash")?;

    // Verbosity 1 lists the block's TXIDs, which proves membership independently.
    let raw_block = client.call(None, "getblock", &[block_hash.clone(), "1".to_owned()])?;
    let transaction_is_in_block = parse_cli_value(&raw_block)?
        .get("tx")
