...
pub fn mine_one_block<C: RpcClient>(client: &C, miner_address: &str) -> LabResult<String> {
    let blocks = crate::labs::lab03_maturity::mine_blocks(client, miner_address, 1)?;
    blocks.into_iter().next().ok_or(LabError::Parse(
        "generatetoaddress returned no block".to_owned(),
    ))
}
...
pub fn mempool_is_empty<C: RpcClient>(client: &C) -> LabResult<bool> {
    Ok(crate::labs::lab05_mempool::get_raw_mempool(client)?.is_empty())
}
...
pub fn confirm_and_locate_transaction<C: RpcClient>(
    ...
    let raw_transaction = client.call(Some(wallet_name), "gettransaction", &[txid.to_owned()])?;
    let transaction = parse_cli_value(&raw_transaction)?;
    ...
    let transaction_is_in_block = block
        .get("tx")
        .and_then(|tx| tx.as_array())
        .ok_or(LabError::MissingField("tx"))?
        .iter()
        .any(|block_txid| block_txid.as_str() == Some(txid));
