...
pub fn confirm_and_locate_transaction<C: RpcClient>(
    ...
    let tx_value = parse_cli_value(&tx_raw)?;
    let block_hash = required_string(&tx_value, "blockhash")?;
    ...
    let block_txids = block_value
        .get("tx")
        .and_then(Value::as_array)
        .ok_or(LabError::MissingField("tx"))?;
    let transaction_is_in_block = block_txids.iter().any(|entry| entry.as_str() == Some(txid));
