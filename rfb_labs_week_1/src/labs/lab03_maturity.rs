...
pub fn mine_blocks<C: RpcClient>(client: &C, address: &str, count: u64) -> LabResult<Vec<String>> {
    ...
    let hashes: Vec<String> = serde_json::from_str(raw.trim())?;
    Ok(hashes)
}
...
pub fn attempt_payment<C: RpcClient>(
    ...
    // `?` deliberately propagates an immature-coinbase failure instead of masking it.
    let raw = client.call(
        Some(wallet_name),
        "sendtoaddress",
...
pub fn demonstrate_coinbase_maturity<C: RpcClient>(
    ...
    // The single coinbase reward is still immature, so this payment must fail.
    let premature_spend_error = match attempt_payment(client, miner_wallet, receiver_address, 1.0) {
        ...
        Ok(txid) => {
            return Err(LabError::Rpc(format!(
                "expected an immature-coinbase rejection, but the payment broadcast as {txid}"
            )))
        }
    };

    // COINBASE_MATURITY is 100, so height 101 makes the first reward spendable.
