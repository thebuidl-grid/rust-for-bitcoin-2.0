...
pub fn mine_blocks<C: RpcClient>(client: &C, address: &str, count: u64) -> LabResult<Vec<String>> {
    ...
    value
        ...
        .ok_or(LabError::Parse(
            "expected an array of block hashes".to_owned(),
        ))?
        .iter()
        .map(|entry| {
            entry
                ...
                .ok_or(LabError::Parse("expected a block hash string".to_owned()))
        })
        .collect()
}
...
pub fn demonstrate_coinbase_maturity<C: RpcClient>(
    ...
    let height_after_first_block = {
        ...
        parse_cli_value(&raw)?.as_u64().ok_or(LabError::Parse(
            "expected a numeric block height".to_owned(),
        ))?
    };
