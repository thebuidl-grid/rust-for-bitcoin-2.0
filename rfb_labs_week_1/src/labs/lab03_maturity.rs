...
pub fn mine_blocks<C: RpcClient>(client: &C, address: &str, count: u64) -> LabResult<Vec<String>> {
    ...
    parse_cli_value(&raw)?
        ...
        .ok_or(LabError::Parse(
            "generatetoaddress did not return block hashes".to_owned(),
        ))?
        .iter()
        .map(|hash| {
            hash.as_str()
                .map(ToOwned::to_owned)
                .ok_or(LabError::Parse("block hash was not a string".to_owned()))
        })
        .collect()
}
    ...
    let premature_spend_error = match attempt_payment(client, miner_wallet, receiver_address, 1.0) {
        Ok(txid) => {
            return Err(LabError::Parse(format!(
                "premature coinbase spend unexpectedly succeeded with txid {txid}"
            )))
        }
