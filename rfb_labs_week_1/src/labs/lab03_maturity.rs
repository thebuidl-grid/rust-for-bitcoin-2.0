...
pub fn attempt_payment<C: RpcClient>(
    ...
    value
        ...
        .ok_or_else(|| LabError::Parse("expected a transaction id".to_owned()))
}

/// Read the node-wide block count.
fn get_block_count<C: RpcClient>(client: &C) -> LabResult<u64> {
    let raw = client.call(None, "getblockcount", &[])?;
    let value = parse_cli_value(&raw)?;
    value
        .as_u64()
        .ok_or_else(|| LabError::Parse("expected a numeric block height".to_owned()))
}
    ...
    let premature_spend_error = match attempt_payment(client, miner_wallet, receiver_address, 1.0) {
        Ok(txid) => {
            return Err(LabError::Rpc(format!(
                "expected the premature payment to fail, but it succeeded with txid {txid}"
            )))
        }
