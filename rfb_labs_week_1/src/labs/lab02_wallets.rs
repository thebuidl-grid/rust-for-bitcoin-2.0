...
pub fn list_wallets<C: RpcClient>(client: &C) -> LabResult<Vec<String>> {
    ...
    value
        .as_array()
        .ok_or_else(|| crate::LabError::Parse("expected an array of wallet names".to_owned()))?
        .iter()
        .map(|v| {
            v.as_str()
                .map(ToOwned::to_owned)
                .ok_or_else(|| crate::LabError::Parse("expected string wallet name".to_owned()))
        })
        .collect()
}
...
pub fn get_new_address<C: RpcClient>(
    ...
    value
        .as_str()
        .map(ToOwned::to_owned)
        .ok_or_else(|| crate::LabError::Parse("expected a string address".to_owned()))
}
...
pub fn address_belongs_to_wallet<C: RpcClient>(
    ...
    value
        ...
        .ok_or_else(|| crate::LabError::Parse("expected ismine field".to_owned()))
}
