...
/// Blocks mined on top of the confirming block to reach the customary six.
const EXTRA_CONFIRMATIONS: u64 = 5;
...
pub fn get_block_header<C: RpcClient>(
    ...
    // `getblockheader` is verbose by default, which is what exposes the commitments.
    ...
    Ok(BlockHeaderEvidence {
        ...
        // The genesis block is the one header with nothing behind it.
...
pub fn build_security_report<C: RpcClient>(
    ...
    // Each additional block extends the chain the payment is buried under, so undoing
    // it would mean redoing that much proof of work faster than the honest network.
    mine_additional_blocks(client, miner_address, EXTRA_CONFIRMATIONS)?;
