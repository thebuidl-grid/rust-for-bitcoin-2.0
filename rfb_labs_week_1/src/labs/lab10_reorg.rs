...
pub fn get_chain_tip<C: RpcClient>(client: &C) -> LabResult<ChainTip> {
    ...
    Ok(ChainTip {
        ...
        // Total work behind this branch, not its length. This is what nodes compare.
...
pub fn reconnect_peer<C: RpcClient>(client: &C, peer_address: &str) -> LabResult<()> {
    // `onetry` attempts a single connection now instead of adding a permanent entry.
    client.call(
        None,
        "addnode",
...
pub fn build_reorg_report(
    ...
    // Convergence means both nodes now name the same block as their tip. Equal
    // heights alone would not do: two branches can be the same length and differ.
