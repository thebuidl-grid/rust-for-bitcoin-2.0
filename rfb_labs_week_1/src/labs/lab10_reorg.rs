...
pub fn get_chain_tip<C: RpcClient>(client: &C) -> LabResult<ChainTip> {
    ...
    Ok(ChainTip {
        ...
        // `chainwork` is the total work behind the tip. It, not the height, is what
        // nodes actually compare when two branches disagree.
...
pub fn build_reorg_report(
    ...
    // Convergence means both nodes now name the same block as their tip. The node that
    // mined the shorter branch reorganized onto the branch carrying the most work.
