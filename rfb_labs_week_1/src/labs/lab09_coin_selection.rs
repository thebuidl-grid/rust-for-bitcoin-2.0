...
const FUNDING_PAYMENTS: usize = 3;

/// Size of each funding payment, chosen so no single UTXO can cover a 1 BTC spend.
const FUNDING_AMOUNT_BTC: f64 = 0.4;

/// Amount Alice forwards, which must therefore combine several funding UTXOs.
...
pub fn create_three_funding_transactions<C: RpcClient>(
    ...
    // Three separate sends create three UTXOs; one larger send would create only one.
    (0..FUNDING_PAYMENTS)
...
pub fn audit_multi_utxo_spend<C: RpcClient>(
    ...
    Ok(MultiUtxoAudit {
        ...
        spend_txid,
        // Inputs are spent whole, so covering 1 BTC needs more than one 0.4 BTC UTXO.
