...
pub fn get_chain<C: RpcClient>(client: &C) -> LabResult<String> {
    ...
    let value = parse_cli_value(&result)?;
...
pub fn get_block_height<C: RpcClient>(client: &C) -> LabResult<u64> {
    let result: LabResult<String> = client.call(None, "getblockcount", &[]);
    result.and_then(|s| s.parse::<u64>().map_err(|e| LabError::Parse(e.to_string())))
}
...
pub fn inspect_network<C: RpcClient>(client: &C) -> LabResult<NetworkSnapshot> {
    ...
    if chain != "regtest" {
        return Err(LabError::Rpc(format!("Unsupported Network: {chain}")));
    }
