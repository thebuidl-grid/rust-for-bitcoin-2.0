...
pub fn get_block_height<C: RpcClient>(client: &C) -> LabResult<u64> {
    ...
    value
        .as_u64()
        .ok_or_else(|| LabError::Parse("getblockcount did not return a number".to_owned()))
}
...
pub fn get_best_block_hash<C: RpcClient>(client: &C) -> LabResult<String> {
    ...
    value
        ...
        .ok_or_else(|| LabError::Parse("getbestblockhash did not return a string".to_owned()))
}
...
pub fn inspect_network<C: RpcClient>(client: &C) -> LabResult<NetworkSnapshot> {
    ...
    if chain != "regtest" {
        return Err(LabError::Rpc(format!(
            "expected a regtest network, but node reports chain `{chain}`"
        )));
    }
