...
pub fn get_block_height<C: RpcClient>(client: &C) -> LabResult<u64> {
    ...
    parse_cli_value(&response)?.as_u64().ok_or_else(|| {
        LabError::Parse("getblockcount did not return a non-negative integer".into())
    })
}
...
pub fn get_best_block_hash<C: RpcClient>(client: &C) -> LabResult<String> {
    ...
    parse_cli_value(&response)?
        ...
        .ok_or_else(|| LabError::Parse("getbestblockhash did not return a hash".into()))
}
...
pub fn inspect_network<C: RpcClient>(client: &C) -> LabResult<NetworkSnapshot> {
    ...
    if chain != "regtest" {
        return Err(LabError::Parse(format!(
            "expected regtest chain, found {chain}"
        )));
    }
