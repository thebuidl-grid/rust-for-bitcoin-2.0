...
pub fn get_raw_mempool<C: RpcClient>(client: &C) -> LabResult<Vec<String>> {
    ...
    let txids: Vec<String> = serde_json::from_str(raw.trim())?;
    Ok(txids)
}
...
pub fn get_transaction_status<C: RpcClient>(
    ...
    Ok(WalletTransactionStatus {
        ...
        // Absent until a block contains the transaction.
        block_hash: value
            .get("blockhash")
...
pub fn observe_unconfirmed_payment<C: RpcClient>(
    ...
    Ok(MempoolObservation {
        ...
        sender_status: get_transaction_status(client, sender_wallet, &txid)?,
        // Broadcast alone leaves the value in `untrusted_pending`, not `trusted`.
        receiver_balance: get_balances(client, receiver_wallet)?,
        txid,
    })
}
