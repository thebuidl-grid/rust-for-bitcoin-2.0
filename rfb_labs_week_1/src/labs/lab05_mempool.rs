...
pub fn send_btc<C: RpcClient>(
    ...
    parse_cli_value(&response)?
        ...
        .ok_or_else(|| LabError::Parse("sendtoaddress did not return a transaction ID".into()))
}
...
pub fn get_transaction_status<C: RpcClient>(
    ...
    let fee = match value.get("fee") {
        Some(fee) => Some(
            fee.as_f64()
                .ok_or_else(|| LabError::Parse("transaction fee was not numeric".into()))?,
        ),
        None => None,
    };
    let block_hash = match value.get("blockhash") {
        Some(block_hash) => Some(
            block_hash
                .as_str()
                .map(ToOwned::to_owned)
                .ok_or_else(|| LabError::Parse("blockhash was not a string".into()))?,
        ),
        None => None,
    };
