...
pub fn identify_payment_and_change(
    ...
    let payment = transaction
        ...
        .ok_or(LabError::Parse(format!(
            "no output pays receiver address {receiver_address}"
        )))?;
...
pub fn calculate_fee(transaction: &DecodedTransaction) -> LabResult<f64> {
    ...
    if fee < 0.0 {
        return Err(LabError::Parse(format!(
            "negative fee is impossible: {fee}"
        )));
    }

    // Round to satoshi precision (1e-8 BTC) to absorb binary floating-point
    // summation drift; Bitcoin amounts never carry more precision than this.
