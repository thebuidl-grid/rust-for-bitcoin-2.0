...
pub fn identify_payment_and_change(
    ...
    let payment = transaction
        ...
        .cloned()
        .ok_or_else(|| LabError::Rpc(format!("no output pays `{receiver_address}`")))?;

    let change = transaction
        .outputs
        .iter()
        .find(|output| output.vout != payment.vout && output.address.is_some())
        .cloned();
...
pub fn calculate_fee(transaction: &DecodedTransaction) -> LabResult<f64> {
    ...
    // Round to satoshi precision (8 decimals) to avoid f64 summation noise:
    // BTC amounts are inherently quantized to satoshis.
    let fee = ((input_total - output_total) * 1e8).round() / 1e8;

    if fee < 0.0 {
        return Err(LabError::Rpc(format!(
            "negative fee: inputs `{input_total}` are less than outputs `{output_total}`"
        )));
    }
