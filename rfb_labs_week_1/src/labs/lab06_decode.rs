...
pub fn calculate_fee(transaction: &DecodedTransaction) -> LabResult<f64> {
    ...
    // Round to satoshi precision (1e-8 BTC) to avoid float-summation noise past the
    // smallest unit Bitcoin actually represents.
    let fee = ((input_sum - output_sum) * 1e8).round() / 1e8;

    if fee < 0.0 {
        return Err(LabError::Parse(
            "calculated a negative fee: outputs exceed inputs".to_owned(),
        ));
    }
