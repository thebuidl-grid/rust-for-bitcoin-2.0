...
#[test]
fn select_utxos_with_exact_match() {
    let available = vec![utxo("a", 100_000)];
...
#[test]
fn select_utxos_empty_list_returns_insufficient_funds() {
...
#[test]
fn select_utxos_maintains_order() {
    let available = vec![utxo("a", 30_000), utxo("b", 40_000), utxo("c", 50_000)];
    let selected = select_utxos(&available, 70_000).unwrap();

    // Should select first two in order
    assert_eq!(selected[0].outpoint.txid, "a");
    assert_eq!(selected[1].outpoint.txid, "b");
}
