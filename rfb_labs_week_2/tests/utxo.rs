...
#[test]
fn selection_stops_when_target_reached() {
    let available = vec![utxo("a", 100_000), utxo("b", 50_000), utxo("c", 30_000)];
...
#[test]
fn selection_with_exact_match() {
    let available = vec![utxo("a", 70_000), utxo("b", 30_000)];
    let selected = select_utxos(&available, 100_000).unwrap();

    assert_eq!(selected.len(), 2);
    assert_eq!(selected.iter().map(|u| u.value).sum::<u64>(), 100_000);
}

#[test]
fn selection_from_empty_slice_fails() {
    let available: Vec<Utxo> = vec![];

    assert_eq!(
        select_utxos(&available, 50_000),
        Err(TransactionError::InsufficientFunds {
            available: 0,
            required: 50_000,
        })
