...
#[test]
fn selection_stops_once_target_is_reached() {
    let available = vec![utxo("a", 40_000), utxo("b", 40_000), utxo("c", 40_000)];
    let selected = select_utxos(&available, 50_000).unwrap();

    assert_eq!(selected.len(), 2);
    assert_eq!(selected.iter().map(|utxo| utxo.value).sum::<u64>(), 80_000);
}
