...
impl Transaction {
    ...
    pub fn validate(&self) -> Result<(), TransactionError> {
        ...
        let coinbase_count = self
            ...
            .count();
        let has_regular_input = coinbase_count < self.inputs.len();

        if coinbase_count > 1 {
            return Err(TransactionError::MultipleCoinbaseInputs);
        }
        if coinbase_count > 0 && has_regular_input {
...
impl fmt::Display for Transaction {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        write!(
            formatter,
            "Transaction v{} (locktime {}): {} input(s) totalling {} sats, {} output(s) totalling {} sats, fee {}",
            self.version,
            self.locktime,
            self.inputs.len(),
            total_inputs,
