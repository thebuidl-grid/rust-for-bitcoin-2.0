...
fn format_output_type(output_type: OutputType) -> &'static str {
    match output_type {
...
impl fmt::Display for TxOutput {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            ...
            self.value,
            self.recipient,
            format_output_type(self.output_type)
        )
    }
}
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            InputKind::Regular {
                ...
                formatter,
                "regular input spending {} ({} sats, sequence {})",
