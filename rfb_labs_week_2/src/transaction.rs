...
impl Transaction {
    ...
    pub fn validate(&self) -> Result<(), TransactionError> {
        ...
        // 2. Outputs exceeding inputs (using ? with fee subtraction)
        let _ = self.fee()?;

        // 3. Mixed coinbase/regular inputs, multiple coinbase inputs, and invalid regular TXIDs
        ...
        if coinbase_count > 0 && has_regular {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            InputKind::Regular {
                ...
                write!(
                    f,
                    "Regular input spending {} ({} sats, seq: {})",
            ...
            InputKind::Coinbase {
                ...
                write!(
                    f,
                    "Coinbase input at height {} (reward: {} sats)",
                    block_height, reward
                )
            }
        }
