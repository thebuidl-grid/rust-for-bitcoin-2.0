...
pub fn demonstrate_coinbase_maturity<C: RpcClient>(
    ...
    // The single coinbase reward is still immature, so this payment must fail. The
    // rejection text is the evidence the lab asks for.
    let premature_spend_error = match attempt_payment(client, miner_wallet, receiver_address, 1.0) {
        ...
        Ok(txid) => {
            return Err(LabError::Parse(format!(
                "premature spend unexpectedly succeeded as {txid}"
            )))
        }
    };
